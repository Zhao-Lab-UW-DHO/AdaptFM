...
from AdaptFM.gui.napari_utils import (
    ParentWindowWatcher,
    reorder_docks,
    restore_all_widgets,
...
def main():
    ...
    for idx, spec in enumerate(widget_specs):
        ...
        action.triggered.connect(
            lambda checked=False, i=idx: restore_or_focus_widget(
                i, widget_specs, viewer
            )
        )
    ...
    restore_all_action.triggered.connect(
        lambda: restore_all_widgets(viewer, widget_specs)
    )
