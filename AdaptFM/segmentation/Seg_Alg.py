...
from AdaptFM.segmentation.checkpoint_utils import (
...
from AdaptFM.segmentation.fourier.nuc_seg_gpu import (
    run_nuclear_segmentation_gpu_chunked,
)
...
class SegmentationAlgorithmSpec(ABC):
    name: str
    mode: str = (
        "batch"  # "batch" | "interactive" — default keeps all existing algos unchanged
    )
...
class NucLogGabor(SegmentationAlgorithmSpec):
    ...
    def tunable_params(self):
        return {
            ...
            "max_freq": {"type": "float", "default": 0.01, "min": 0.001, "max": 1.0},
            "frequency_step": {"type": "float", "default": 0.01, "min": 0, "max": 1.0},
    ...
    def run(self, volume, params):
        ...
        for z in range(volume.shape[0]):
            thresh = threshold_sauvola(
                volume[z], window_size=params["window_size"], k=params["k"]
            )
...
class Felzenszwalb3D(SegmentationAlgorithmSpec):
    ...
    def run(self, volume, params):
        ...
        for z in range(volume.shape[0]):
            slice_labels = felzenszwalb(
                volume[z], scale=params["scale"], sigma=params["sigma"]
            )
...
class SAM2ClickAndPropagate(SegmentationAlgorithmSpec):
    ...
    def initialize(self, volume: np.ndarray, params: dict | None = None) -> None:
        """
    ...
    def propagate(
        ...
        if direction in ("forward", "both"):
            self._run_propagation(
                reverse=False, threshold=threshold, multimask=multimask
            )

        if direction in ("backward", "both"):
            self._run_propagation(
                reverse=True, threshold=threshold, multimask=multimask
            )

        return self._label_vol.copy()

    def _run_propagation(
        self, reverse: bool, threshold: float, multimask: bool
    ) -> None:
    ...
    def _load_predictor(self, model_size: str, gpu: int) -> None:
        ...
        if model_size not in cfg_map:
            raise ValueError(
                f"Unknown model_size '{model_size}'. Choose from {list(cfg_map)}"
            )
        ...
        self._predictor = build_sam2_video_predictor(
            cfg_file, str(ckpt_path), device=device
        )
...
class SAM3TextAndPropagate(SegmentationAlgorithmSpec):
    ...
    def initialize(self, volume: np.ndarray, params: dict | None = None) -> None:
        ...
        os.environ["SAM3_REPO_ROOT"] = str(
            REPO_ROOT / "segmentation" / "sam3" / "sam3"
        )  # path to this repo
    ...
    def add_prompt(
        ...
        pts_rel = [[px / self._img_w, py / self._img_h] for px, py in pts_abs]

        device = torch.device(f"cuda:{self.gpu}")

        points_tensor = torch.tensor(pts_rel, dtype=torch.float32).to(device)

        labels_tensor = torch.tensor(labels_raw, dtype=torch.int32).to(device)
        ...
        union_mask = np.zeros((self._img_h, self._img_w), dtype=bool)
        ...
        if instance_masks is not None:
            ...
            self._active_obj_ids = [
                int(oid.item() if hasattr(oid, "item") else oid) for oid in sam_ids
            ]
    ...
    def add_text_prompt(
        ...
        self._active_obj_ids = [
            oid.item() if hasattr(oid, "item") else int(oid) for oid in detected_obj_ids
        ]
        #######################
        ...
        union_mask = np.zeros(self._volume.shape[1:], dtype=bool)
        sam_ids = result.get("out_obj_ids", [])
    ...
    def add_text_and_point_prompt(
        ...
        if mask is not None:
            self._label_vol[z] = np.where(mask, obj_id, self._label_vol[z])
        return (
            mask if mask is not None else np.zeros(self._volume.shape[1:], dtype=bool)
        )
    ...
    def propagate(self, obj_id=1, direction="both", params=None):
        ...
        obj_ids_to_propagate = (
            self._active_obj_ids if self._active_obj_ids else [obj_id]
        )
        ...
        with torch.inference_mode(), torch.autocast(device_type="cuda"):
            for frame_output in self._predictor.handle_stream_request(
                ...
                for i, oid in enumerate(out_obj_ids):
                    ...
                    mask = binary_masks[i]
                    mask_np = (
                        mask.cpu().numpy() if hasattr(mask, "cpu") else np.array(mask)
                    )
    ...
    def _load_predictor(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "SAM3 is not installed. You must install using the environment manager`"
            ) from e
        ...
        self._predictor = build_sam3_video_predictor(
            checkpoint_path=ckpt_path, gpus_to_use=[self.gpu]
        )
    ...
    def _result_to_mask(
        ...
        thresh = (
            ...
            else params.get("score_threshold", 0.5)
        )
        ...
        scores_np = np.array(
            [p.item() if hasattr(p, "item") else float(p) for p in probs]
        )  # (N,)
    ...
    @staticmethod
    def _tensor_to_mask(obj_output: dict, threshold: float) -> np.ndarray | None:
        """
