...
def get_3d_power_spectrum(image):
    ...
    power_spectrum = (
        np.abs(fft_image) ** 2
    )  # square the shifted fft to get the power spectrum
...
def detect_knee_3d(radial_frequencies, radial_power):
    ...
    nonzero_power_indices = (
        radial_power > 0
    )  # remove zero power regions b/c they are uninformative and can mess with knee detection
...
def apply_gaussian_low_pass_filter_3d(grayscale_image, cutoff, z_scaling=1.0):
    ...
    distance = np.sqrt(
        ((X - ccol) ** 2) + ((Y - crow) ** 2) + ((Z - cdepth) ** 2) * (z_scaling**2)
    )
...
def run_nuclear_segmentation(
    volume, percentile, max_freq, frequency_step, sigma, remove_background: bool
):
