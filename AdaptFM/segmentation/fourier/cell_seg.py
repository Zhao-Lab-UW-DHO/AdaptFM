...
def radial_average_high_pass(power_spectrum):
    ...
    nonzero_indices = (
        sorted_power_spectrum > 0
    )  # remove zero power cause it contains nothing meaningful
...
def detect_knee_high_pass(radial_frequencies, radial_power):
    ...
    knee_locator = KneeLocator(
        radial_frequencies, radial_power, curve="convex", direction="decreasing"
    )
...
def run_single_cell_segmentation(volume, minimum_size, percentile):
    ...
    for z in range(num_z_layers):
        ...
        high_passed_image = apply_high_pass_filter(
            grayscale_z, cutoff=knee_frequency_high_pass
        )

        # threshold the layer based on the threshold defined above
        final_image[z, :, :] = np.where(
            high_passed_image > np.percentile(high_passed_image, percentile), 1, 0
        )
