"""
install-cellposesam: creates the ReScale4DL conda environment, installs ReScale4DL,
then swaps in the user's custom PyTorch build.

Usage (after `pip install -e .`):
    install-rescale4dl

ReScale4DL pulls in a default PyTorch (often CUDA 13 / latest) that most users
...
def main() -> None:
    print(f"\n=== Installing ReScale4DL environment: {ENV_NAME} ===\n")
    ...
    if ENV_NAME in env_check.stdout:
        ...
        print("To reinstall from scratch, run:  conda env remove -n rescale4dl_adapt")
        sys.exit(0)
    ...
    print("\n--- Installing rescale4dl ---")
    _conda_run(ENV_NAME, ["python", "-m", "pip", "install", "rescale4dl"])


    print(f"\n✓ rescale4dl environment '{ENV_NAME}' created successfully.")
