...
from AdaptFM.model.fmSpec import (
    BMEXSpec,
    CellposeSAMSpec,
    CellSAMSpec,
    MicroSAMSpec,
...
from AdaptFM.model.nnUNetV2Spec import MerlinNNUNetV2ModelSpec, NNUNetV2ModelSpec
...
MODEL_REGISTRY = {
    "nnUNetv2": NNUNetV2ModelSpec(conda_env=_read_prefix("nnUNet_adapt")),
    "microSAM": MicroSAMSpec(
        name="microSAM",
        conda_env=_read_prefix("micro-sam_adapt"),
        module_path="micro_sam.training",
        training_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "microSAM" / "train_wrapper.py"
        ),
        inference_wrapper_path=str(
            ADAPTFM_MODEL_PATH
            / "foundation_models"
            / "microSAM"
    ...
    "SAMMed3D": Sammed3DSpec(
        ...
        training_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "sammed3d" / "train_wrapper.py"
        ),
        inference_wrapper_path=str(
            ADAPTFM_MODEL_PATH
            / "foundation_models"
    ...
    "SSVT": SSVTSpec(
        ...
        training_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "SSVT" / "train_wrapper.py"
        ),
        inference_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "SSVT" / "inference_wrapper.py"
        ),
    ...
    "BMEX": BMEXSpec(
        ...
        training_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "bmex" / "train_wrapper.py"
        ),
        inference_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "bmex" / "inference_wrapper.py"
        ),
    ...
    "CT FM": CTFMSpec(
        ...
        training_wrapper_path="",
        inference_wrapper_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "ctfm" / "inference_wrapper.py"
        ),
        training_function="launch_training",
    ),
    "Merlin nnUNet": MerlinNNUNetV2ModelSpec(
        ...
        transform_path=str(
            ADAPTFM_MODEL_PATH / "foundation_models" / "merlin" / "transforms.py"
        ),
