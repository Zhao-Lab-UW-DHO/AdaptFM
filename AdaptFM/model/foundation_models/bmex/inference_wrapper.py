...
from utils.utils import fill_holes, reorient_to_std
...
def write_sidecar_json(
    nifti_path: str,
    sources_paths: list,
    spatial_ref_path: str,
    bids_root_dir: str,
    skull_stripped: bool,
    qi_value: float | None = None,
        ...
        "GeneratedBy": [
            {
                "Name": "BME-X",
                "Version": "v1.0.5",
                "Container": {"Type": "docker", "Tag": "yuesun814/bme-x:v1.0.5"},
            }
        ],
    }
...
def main():
    ...
    with torch.no_grad():
        bids_root_path = (
            args.test_dir
        )  # users need to enter the root for the BID dataset
        ...
        for t1w_file in t1w_files:
            ...
            size, origin, spacing, direction = (
                T1w_img_reorient.GetSize(),
                T1w_img_reorient.GetOrigin(),
                T1w_img_reorient.GetSpacing(),
                T1w_img_reorient.GetDirection(),
            )
            ...
            T1w_img_reorient_downsample = sitk.Cast(
                T1w_img_reorient_downsample, sitk.sitkFloat32
            )
            ...
            T1w_img_reorient_downsample_hm = matcher.Execute(
                T1w_img_reorient_downsample, template
            )

            print("Skull stripping")
            T1w_img_reorient_downsample_hm = sitk.GetArrayFromImage(
                T1w_img_reorient_downsample_hm
            )
            T1w_img_reorient_downsample_hm = torch.tensor(
                T1w_img_reorient_downsample_hm
            ).float()
            T1w_img_reorient_downsample_hm = T1w_img_reorient_downsample_hm.unsqueeze(
                dim=0
            )
            T1w_img_reorient_downsample_hm = T1w_img_reorient_downsample_hm.unsqueeze(
                dim=0
            ...
            logits = sliding_window_inference(
                T1w_img_reorient_downsample_hm, (64, 64, 64), 1, model1, overlap=0.85
            )
            promap = logits[:, :2, :, :, :]
            ...
            promap_upsampled = F.interpolate(
                promap,
