...
***In AdaptFM Otsu thresholding uses one threshold value for the entire 3D volume.***
...
1. **Examine the image intensities** - the algorithm looks at the range of pixel intensities throughout the image and determines how frequently each pixel intensity occurs
...
3. **Separate the image into two groups** - For each possible threshold, pixels are divided into two groups:
    ...
    - Pixels above the threshold are made foreground
...
1. **Remove Small Objects** - specifies the minimum size of regions that should be retained after thresholding
    ...
    - Higher value - removes more small regions and produces an image segmentation with fewer objects
