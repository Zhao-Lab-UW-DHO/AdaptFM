...
***In AdaptFM, the algorithm is applied independently in every 2D z-slice. The resulting masks from all slices are then combined to produce a final 3D segmentation***
...
3. **Remove broad image features** - a 'high-pass' filter is applied to remove low-frequency information such as gradual changes in brightness, large-scale background variation, and other broad structures. 
...
2. **Minimum size** - controls which detected regions are retained based on their size
    ...
    - Higher minimum size - removes more small regions and suppresses small noise
